...
b_w=0.00001;%排他制約の重み係数
d_w=0.001;%エリア間電力融通、配電損失重み係数
...
%% 目的関数
% C=f_w*ones(nPeriods,nArea*2);%電力量変数設定、排他条件設定
% C=[C;zeros(nPeriods,factorial(nArea));].';
% C=[C(:);ones(nPeriods,1);];%変数z（目的関数）
% f=[f f f f];
...
C=A_f_1;
d=b_f_1;
C=[C;b_w*ones(1,72) -b_w*ones(1,72) zeros(1,168)];
d=[d;0];
%制約条件まとめ
sw_c=1;
sw_l=1;
...
tic
[x,fval,eflag,out] = lsqlin(C,d,A,b,Aeq,beq,lb,ub,options);
...
%% 解の分解整理
if isempty(fval)==0
outx=zeros(nPeriods,(numel(x)-nPeriods)/nPeriods);
for n=1:(numel(x)-nPeriods)/nPeriods
    for h=1:nPeriods
