...
pv_out=pv_out/2;%常に晴ということは無いので、半分に
% pv_out=1*ones(24,1);%常に1
%pv_out=[pv_out(1:9);1*ones(15,1)];%10番目から常に1
pv_out=[pv_out 5*pv_out 5*pv_out];
...
A1_tril=cat(2,A_tril,zero_1,zero_1,-A_tril,zero_1,zero_1,A_tril,zero_1,zero_1,zero_1,zero_1,A_tril);
A2_tril=cat(2,zero_1,A_tril,zero_1,zero_1,-A_tril,zero_1,zero_1,A_tril,zero_1,A_tril,zero_1,zero_1);
A3_tril=cat(2,zero_1,zero_1,A_tril,zero_1,zero_1,-A_tril,zero_1,zero_1,A_tril,zero_1,A_tril,zero_1);
A1_eye=cat(2,Aeq_eye,zero_1,zero_1,-Aeq_eye,zero_1,zero_1,-Aeq_eye,zero_1,Aeq_eye,Aeq_eye,zero_1,-Aeq_eye);
A2_eye=cat(2,zero_1,Aeq_eye,zero_1,zero_1,-Aeq_eye,zero_1,Aeq_eye,-Aeq_eye,zero_1,-Aeq_eye,Aeq_eye,zero_1);
A3_eye=cat(2,zero_1,zero_1,Aeq_eye,zero_1,zero_1,-Aeq_eye,zero_1,Aeq_eye,-Aeq_eye,zero_1,-Aeq_eye,Aeq_eye);
...
figure_out_bar('ネットロード',netload,[0 25],[0 3000],'Time [hour]','netload[kWh]',[1.75 0.1 0.25 0.3])
%figure_out_bar('result',ev_out,[0 25],[0 1],'Time [hour]','evout [kWh]')
figure_out_plot('容量推移',capx,[1 25],[0 battery_capacity],'Time [hour]','capacity [kWh]',[1.5 0.1 0.25 0.3])
%figure_out_plot('result',netload,[0 24],[-3 3],'Time [hour]','netload [kWh]')
figure_out_bar('最適化前',demand_data+ev_out*Area_ev.*Area_demand,[0 25],[0 3000],'Time [hour]','load(with ev) [kWh]',[1.25 0.1 0.25 0.3])
figure_out_bar('最適化後',after_flow,[0 25],[0 3000],'Time [hour]','afterflow [kWh]',[1.0 0.1 0.25 0.3])
